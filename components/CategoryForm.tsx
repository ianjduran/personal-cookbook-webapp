    ...
    <div>
      ...
      <div className='flex flex-wrap gap-8 md:flex-nowrap'>
        ...
        <form onSubmit={handleSubmit} {...props} className=''>
          ...
          <div className="mb-4">
            <label className="block text-lg font-bold  font-subtitle">Nombre</label>
            <input required className="px-2 py-1 border rounded-md shadow-md dark:text-slate-800 font-subtitle caret-blue-500 focus:outline-blue-500 focus:border-blue-500"
              type="text" placeholder="Ejemplo: Pasteleria" onChange={e => setCategoryName(e.target.value)} />
          </div>

          {/* setIcon de hijo a padre */}
          {/* <IconPicker onChange={setIcon}/> || selected -> icono as html tag */}
          <div>
            <label className="block text-lg font-bold  font-subtitle">Icono</label>
          ...
          <div>
            <label className="block text-lg font-bold  font-subtitle">Color</label>
            <ColorPicker onChange={setColorIcon} />
          </div>
          <button disabled={isSaving} className='px-2 py-1 my-2 bg-gray-50 rounded-md hover:brightness-95 dark:hover:brightness-105 dark:bg-gray-800 text-green-500 disabled:text-gray-500'>
            <div className='flex-inline flex align-middle justify-center items-center'>
              <FiSave className='inline-block mr-2'/>
              <span className='inline-block'>Guardar</span>
            </div>
            </button>
          {uploadState && <p className='text-blue-500 font-subtitle'>{uploadState}</p>}
        </form>

        <div>
          <h1 className='text-md tracking-wide font-medium'>Previsualización</h1>
